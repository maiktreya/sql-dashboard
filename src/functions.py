...
def extract_tables(sql):
    ...
    for from_table, _, from_alias, join_table, _, join_alias, on_left, on_right in matches:
        table_name = from_table or join_table
        alias = from_alias or join_alias or table_name
        if table_name:
            tables[alias] = table_name
        if on_left and on_right:
            joins.append((on_left.split('.')[0], on_right.split('.')[0], f"{on_left} = {on_right}"))
...
def generate_graphviz_dot(tables, joins):
    ...
    for left_table, right_table, condition in joins:
        ...
        left_alias = next((alias for alias, t in tables.items() if t == left_table or alias == left_table), None)
        right_alias = next((alias for alias, t in tables.items() if t == right_table or alias == right_table), None)
        if left_alias and right_alias:
            dot_string += f'  "{left_alias}" -> "{right_alias}" [label="{condition}"];\n'
    dot_string += "}\n"
    return dot_string
